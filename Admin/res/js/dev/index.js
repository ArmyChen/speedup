...
//刷新缓存
$('#cache_submit_btn').on('click', function(e) {
	e.preventDefault();
    $('#cache_submit_btn').attr('disabled',"true");
    $.ajax({url:"?do=cache", async:false, dataType: "json", success:function(result){
	    if(result.state == 'success') {
			layer.alert('缓存更新成功', function(index){
                $('#cache_submit_btn').removeAttr("disabled");
				layer.close(index);
			});
	    }
    }});
